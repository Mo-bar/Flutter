...
class Logos extends StatefulWidget {
  final String email;
  const Logos({Key? key, required this.email}) : super(key: key);
  @override
...
class _LogosState extends State<Logos> {
  ...
  loadData()async{
    List<Map> helper = [];
    helper.addAll(await sql.getdata('SELECT G.*,C.flag, U.fname FROM Gig G, User U,Country C WHERE G.catId = 0 AND G.email = U.email AND C.name = U.countryName'));
    return helper;
  }
@override
  void initState() {
