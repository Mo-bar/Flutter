...
class Website extends StatefulWidget {
  final String email;
  const Website({Key? key, required this.email}) : super(key: key);
  @override
  State<Website> createState() => _WebsiteState();
}
// [{catId: 1, catName: Website}, {catId: 2, catName: Web site}, {catId: 3, catName: Mobile app}, {catId: 4, catName: T-Shirt}]
class _WebsiteState extends State<Website> {
  ...
  loadData()async{
    List<Map> helper = [];
    helper.addAll(await sql.getdata('SELECT G.*,C.flag, U.fname FROM Gig G, User U,Country C WHERE G.catId = 1 AND G.email = U.email AND C.name = U.countryName'));
    return helper;
  }
@override
  void initState() {
